
<?php include('layout-header.php'); ?> 

<div class="parallax bg-white page-section" style="padding: 20px 0;">
    <div class="container parallax-layer" data-opacity="true">
        <div class="media v-middle">
            <div class="media-body">
                <h1 class="text-display-2 margin-none"></h1> 
            </div>
            <!--<div class="media-right">
                <div class="width-100 text-right">
                    <div class="btn-group">
                        <a  class="btn btn-white" href="website-directory-grid.html"><i class="fa fa-th"></i></a>
                        <a  class="btn btn-grey-900" href="website-directory-list.html"><i class="fa fa-list"></i></a>
                    </div>
                </div>
            </div>-->
        </div>
    </div>
</div>

<div class="container">

    <div class="page-section">
        <div class="row">
 
                <div class="tabbable tabs-blocks">

                <!-- Tabs -->
                <ul class="nav nav-tabs" tabindex="4" style="overflow: hidden; outline: none;">
                    
                </ul>
                <!-- // END Tabs -->

                <!-- Panes -->
                <div class="tab-content">  
                    <div id="blocks-settings-1" class="tab-pane active">
                        
                                                <

                                                    <div class="timeline-body" style=" border: 1px solid #dfdfdf;  padding: 15px; margin-bottom: 10px;  background-color: #eee; ">
                                                        <h4>
                                                            <strong>
                                                                <i class="fa fa-download"></i> <a href="../assets/uploads/resources/" download="";></a>
                                                            </strong>   
                                                            <small>
                                                                | added on 
                                                            </small>
                                                        </h4>

                                                        <p> . . .</p>
                                                    </div> 
                                            


                    </div>
                </div>
                <!-- // END Panes -->

            </div>

 

        </div>
    </div>

</div>

<?php include('layout-footer.php'); ?>